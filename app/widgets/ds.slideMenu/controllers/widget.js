...
$.movableview.addEventListener('touchend', function(e) {
	...
	if ($.movableview.left >= "150dp" && touchRightStarted) {
		direction = "right";
		$.leftButton.touchEnabled = false;
		$.movableview.animate(animateRight);
		hasSlided = true;
	}
	else if ($.movableview.left <= "-150dp" && touchLeftStarted) {
		direction = "left";
...
$.movableview.addEventListener('touchmove', function(e) {
	...
	if ((touchRightStarted && newLeft <= "250dp" && newLeft >= 0) || 
		(touchLeftStarted && newLeft <= 0 && newLeft >= "-250dp")) {
		...
		else if (touchRightStarted && newLeft > "250dp") {
			$.movableview.left = "250dp";
		}
		else if (touchLeftStarted && newLeft < "-250dp") {
			$.movableview.left = "-250dp";
		}
	}
